...
## My Thoughts
The first thing we do is split the strings into lines. For each line, remove the leading `-` and at least one following space. We then wrap each cleaned item in `<li>...</li>`. Then we wrap the whole thing in` <ul>...</ui>`.

## Solution (Python)
```python
def parse_unordered_list(md:  str) -> str:
	lines = md.split("\n")
	items_html = []
